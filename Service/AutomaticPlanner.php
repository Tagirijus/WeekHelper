...
class AutomaticPlanner extends Base
{
    ...
    )
    {
        $distributor = new DistributionLogic(
            ...
            // if ignoring "now", means the plan should be created as if
            // nothing started yet. so basically a new plan like the week
            // did not start etc. otherwise the TimePoint string '' will be
            // used, which will be generated by TimePoint form the "now"
            // time.
            // TLDR: "ignore_now" makes TasksPlan get the info as if is
            // Monday 0:00 o'clocl, while "not ignoring now" means that
            // TasksPlan gets "now" as the actual time.
            $ignore_now ? 'mon 0:00' : ''
        );
        if (!$ignore_now) {
